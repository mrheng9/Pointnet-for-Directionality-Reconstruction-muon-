...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='/disk_pool1/houyh/experiments/test43', help='experiment root')
...
def load_data():
    ...
    #coords = np.load('/disk_pool1/houyh/coords/processed_coords%s.npy'%(args.num_point))
    coords = np.load('/disk_pool1/houyh/coords/norm_coords')
    #sample_indices = np.load('/disk_pool1/houyh/coords/sample_indices%s.npy'%(args.num_point))
    #print('sample_indicies:',sample_indices.shape)
    ...
    # sample_indices = sample_indices[:batch_size]
    # sampled_x_all = np.array([x_all[i, sample_indices[i], :] for i in range(len(sample_indices))])
    x_all = np.concatenate([coords,x_all],axis=-1)
    print("shape of x_all:",x_all.shape) 
...
def main(args):
    ...
    #train_dataset = PMTDataLoader2(points_train, labels_train, args,process_data=True)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)

    test_dataset = CustomDataset(points_test, labels_test)
    #test_dataset = PMTDataLoader2(points_test, labels_test, args,process_data=True)
